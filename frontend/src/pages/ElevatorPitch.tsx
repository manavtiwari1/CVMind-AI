...
export default function ElevatorPitch({ customApiKey, resumeText, loadedWork, setLoadedWork }: ElevatorPitchProps) {
  ...
  const handleSaveSession = async () => {
    ...
    const userStr = localStorage.getItem('cvmind_user');
    if (!userStr) {
      alert('Please sign in to save your pitches.');
      return;
    }
          ...
          <div className="elevator-pitch-results-grid animate-fade-in">
            ...
            <div className="results-sidebar glass-card">
              ...
              <button 
                ...
                style={{ background: 'var(--color-emerald)', borderColor: 'var(--color-emerald)', width: '100%', justifyContent: 'center', display: 'flex', marginTop: '1rem' }}
                onClick={handleSaveSession}
                disabled={savingWork}
              >
                {savingWork ? 'Saving...' : saveSuccess ? 'Saved to Works!' : 'Save Pitches'}
              </button>
            </div>
