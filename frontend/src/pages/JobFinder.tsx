  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'card') {
      if (!cardNumber || !cardExpiry || !cardCvv || !cardName) {
        setPaymentError('Please fill in all credit/debit card fields.');
        setProcessingPayment(false);
        return;
      }
    } else if (paymentMethod === 'upi') {
      if (!upiId || !upiId.includes('@')) {
        setPaymentError('Please enter a valid UPI ID (e.g. username@bank).');
        setProcessingPayment(false);
        return;
      }
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(resData.error || 'Failed to process payment checkout.');
  ...
  if (!hasAccess) {
    return (
      <div className="jf-page animate-fade-in-up" style={{ minHeight: '85vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
        ...
        {paymentSuccess ? (
          <div className="glass-card" style={{ maxWidth: '500px', width: '100%', padding: '3.5rem 2rem', textAlign: 'center', borderRadius: '24px', border: '1px solid rgba(48, 209, 88, 0.3)', boxShadow: '0 0 40px rgba(48, 209, 88, 0.15)', position: 'relative' }}>
            <div className="animate-scale-up" style={{ width: '70px', height: '70px', background: 'rgba(48, 209, 88, 0.1)', border: '1px solid rgba(48, 209, 88, 0.4)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem' }}>
              <Sparkles size={32} style={{ color: '#30d158' }} />
            </div>
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#fff', marginBottom: '0.75rem' }}>Payment Successful!</h2>
            <p style={{ color: '#94a3b8', fontSize: '0.95rem', lineHeight: 1.6 }}>
              Thank you! Your transaction has been verified. Unlocking your AI Job Finder Dashboard now...
            </p>
          </div>
        ) : (
          <div className="glass-card" style={{ maxWidth: '580px', width: '100%', padding: '2.5rem 2rem', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: '1.5rem', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
            
            {/* Header */}
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: '56px', height: '56px', background: 'rgba(41, 151, 255, 0.08)', border: '1px solid rgba(41, 151, 255, 0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
                <Lock size={24} style={{ color: '#2997ff' }} />
              </div>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fff', margin: '0 0 0.5rem' }}>Unlock AI Job Finder Premium</h2>
              <p style={{ color: '#94a3b8', fontSize: '0.85rem', lineHeight: 1.5, margin: 0 }}>
                Get instant access to match your CV with 15+ job matches from start-ups to top MNCs, including direct multi-platform apply buttons.
              </p>
            </div>

            {/* Price Badge */}
            <div style={{ background: 'linear-gradient(135deg, rgba(41, 151, 255, 0.08) 0%, rgba(191, 90, 242, 0.08) 100%)', border: '1px solid rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '12px', textAlign: 'center' }}>
              <span style={{ fontSize: '0.75rem', color: '#a1a1aa', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Premium Access Fee</span>
              <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#fff', marginTop: '0.25rem' }}>₹200 <span style={{ fontSize: '0.9rem', color: '#a1a1aa', fontWeight: 500 }}>INR (One-Time)</span></div>
            </div>

            {/* Payment Method Selector */}
            <div>
              <label className="form-label" style={{ marginBottom: '0.5rem', display: 'block' }}>Choose Payment Mode</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem' }}>
                <button
                  type="button"
                  onClick={() => { setPaymentMethod('card'); setPaymentError(null); }}
                  className={`jf-filter-pill ${paymentMethod === 'card' ? 'active' : ''}`}
                  style={{ margin: 0, padding: '0.6rem 0' }}
                >
                  💳 Card
                </button>
                <button
                  ...
                  className={`jf-filter-pill ${paymentMethod === 'upi' ? 'active' : ''}`}
                  style={{ margin: 0, padding: '0.6rem 0' }}
                >
                  📱 UPI
                </button>
                <button
                  ...
                  className={`jf-filter-pill ${paymentMethod === 'paypal' ? 'active' : ''}`}
                  style={{ margin: 0, padding: '0.6rem 0' }}
                >
                  🅿️ PayPal
                </button>
            ...
            <form onSubmit={handleCheckout} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {paymentMethod === 'card' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  ...
                  <div style={{ background: 'linear-gradient(135deg, #1d4ed8 0%, #7c3aed 100%)', padding: '1.25rem', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 8px 20px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '140px', color: '#fff' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.5px' }}>PREMIUM MEMBER</span>
                      <span style={{ fontSize: '0.75rem', fontWeight: 500, opacity: 0.8 }}>CREDIT/DEBIT</span>
                    </div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 600, letterSpacing: '2px', fontFamily: 'monospace', margin: '0.5rem 0' }}>
                      {cardNumber ? cardNumber.replace(/(\d{4})/g, '$1 ').trim() : '•••• •••• •••• ••••'}
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem' }}>
                      <div>
                        <div style={{ fontSize: '0.6rem', opacity: 0.6, textTransform: 'uppercase' }}>Card Holder</div>
                        <div style={{ fontWeight: 600 }}>{cardName || 'JOHN DOE'}</div>
                      </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  <div className="form-group" style={{ margin: 0 }}>
                    ...
                    <input
                      type="text"
                      className="form-input auth-field"
                      placeholder="e.g. user@okhdfcbank or phonephone@ybl"
                      value={upiId}
                  ...
                  <p style={{ color: '#94a3b8', fontSize: '0.75rem', lineHeight: 1.4, margin: 0 }}>
                    Enter your VPA (Virtual Payment Address). A payment request of ₹200 will be simulated on your UPI app dashboard.
                  </p>
                </div>
              )}

              {paymentMethod === 'paypal' && (
                <div style={{ padding: '1rem', border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', textAlign: 'center' }}>
                  <p style={{ color: '#94a3b8', fontSize: '0.85rem', lineHeight: 1.5, margin: '0 0 1rem' }}>
                    You will be redirected to PayPal sandbox window to authorize the ₹200 transaction.
                  </p>
                  <div style={{ background: '#ffc439', color: '#111', fontWeight: 700, padding: '0.6rem', borderRadius: '8px', fontSize: '0.85rem', display: 'inline-flex', alignItems: 'center', gap: '0.25rem' }}>
                    <i>PayPal</i> Checkout
                  </div>
                </div>
              )}
              ...
              {paymentError && (
                <div className="jf-error" style={{ margin: 0 }}>
                  <AlertCircle size={14} />
              ...
              <button
                ...
                style={{ margin: '0.5rem 0 0', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem' }}
                disabled={processingPayment}
              >
                {processingPayment ? (
                  <span className="auth-spinner" style={{ width: '16px', height: '16px' }}></span>
                ) : (
                  <>Pay ₹200 Now & Unlock</>
                )}
              </button>
